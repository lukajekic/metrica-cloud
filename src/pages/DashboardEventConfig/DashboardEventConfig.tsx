...
const getPages = async()=>{
    let pages = await axios.post(`${import.meta.env.VITE_BACKEND}/metrica/event/get`, {
...
setTableData(pages.data.map((item:Page,index:number)=>{
    ...
    toast('Greska prilikom fetchovanja eventova')
  }
  }
...
const deletePage = async(pageID:string) =>{
  try {
    const response = await axios.delete(`${import.meta.env.VITE_BACKEND}/metrica/event/${sessionStorage.getItem('ActiveProject')}/${pageID}`)

    if (response.status === 200) {
      getPages()
      toast.success('Deleted event successfuly')
...
const handleCreation = async()=>{
  setcreation400err("")
  try {
    if (!creationFormData.description || !creationFormData.title) {
      return
    }

    const response = await axios.post(`${import.meta.env.VITE_BACKEND}/metrica/event`, {
...
let updateOBJ = {}
  updateOBJ = {
    ...
    const response = await axios.put(`${import.meta.env.VITE_BACKEND}/metrica/event`, updateOBJ)
  ...
  return (
    ...
    <div id='dashboard-page-config'>
        ...
        <MainContent enforceProtection>
          <DashboardPageTitle title='Events Configuration' subtitle='Setup new and view existing tracked events.'></DashboardPageTitle>
          <div className="w-full flex flex-col items-end gap-3">
            <Button onClick={()=>{setOpenCreationDialog(true)}}><PlusSquare></PlusSquare> Create new Event</Button>
            <DashboardBlock title='My Events'>
     ...
     <AlertDialog open={openDeletionDialog}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete event and all analytics linked to it.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
    ...
    <form onSubmit={(e) => {[
      ...
      <div className="grid gap-4 py-4">
        <div className="grid gap-3">
         ...
         <div className="grid gap-3">
          <Label htmlFor="create-title">Event Description</Label>
          <Input
            id="create-title" 
            name="description" 
            type="text" 
...
export default DashboardEventConfig