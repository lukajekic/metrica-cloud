...
setData(response.data)
  ...
  useEffect(()=>{
    const getPagesData = async()=>{
        try {
        const response = await axios.post(`${import.meta.env.VITE_BACKEND}/metrica/event/get`, {
            projectID: sessionStorage.getItem('ActiveProject')
        })

        if (response.status === 200) {
            let pathToTitleMap = Object.fromEntries(response.data.map(item=>[item.eventID, item.title]))
...
const proccessTableDat = ()=>{
    let allpaths = data.map(item => item.eventID)
    let uniquepaths = [...new Set(allpaths)]

let viewsPerPath = {}
let uq_viewsPerPath = {}

    uniquepaths.forEach(eventID =>{
        const allpathdata = data.filter(item => item.eventID === eventID)
        let totalviews = 0
        let totalunique = 0

        allpathdata.forEach(itemw=>{
            const sum = Object.values(itemw.triggers).reduce((acc, curr)=>acc+curr,0)
            const sum2 = Object.values(itemw.uniqueTriggers).reduce((acc, curr)=>acc+curr,0)
            totalviews += sum
            totalunique += sum2
        })

viewsPerPath[eventID] = (viewsPerPath[eventID] || 0) + totalviews
uq_viewsPerPath[eventID] = (uq_viewsPerPath[eventID] || 0) + totalunique
...
let alleventdatas = data.map(item => item.eventData)

        let tableData = Object.entries(viewsPerPath)
  .map(([key, value]) => ({
    eventID: key,
    triggers: value,
    uniqueTriggers: uq_viewsPerPath[key],
    eventData: alleventdatas.find(item => item._id === key)
  }))
...
proccessTableDat()
  ...
  useEffect(()=>{
console.log(tabelOptimizedData)
  }, [tabelOptimizedData])
  ...
  return (
    ...
    <MainContent enforceProtection>
        <DashboardPageTitle title='Top Events' subtitle='View your most triggered Events'></DashboardPageTitle>
        <DashboardBlock>
...
export default DashboardEventsTop