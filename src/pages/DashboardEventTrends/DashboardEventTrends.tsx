...
getPagesData()
    ...
    useEffect(()=>{
if (rawData.length > 0) {
    const countries = Array.from(new Set(rawData.flatMap((item)=>Object.keys(item.triggers))))
...
const processData = (filtered_local:boolean)=>{
    if (filtered_local === true) {
        ...
        participatingDates.forEach(date => {
            const pw_items = rawData.filter(item => item.date === date && item.eventData._id === filters.eventID)
            let date_views_sum = {}
...
const clearFilters = ()=>{
  ...
  return (
    ...
    <MainContent enforceProtection>
        <DashboardPageTitle title='Event Trigger Trends' subtitle='Analyze event triggers for your project, either combined or by specific parameters.'></DashboardPageTitle>
        <DashboardBlock>
        ...
        <SelectGroup>
          {CountriesList.map(item => (
            <SelectItem key={item.code} value={item.code}>{item.name}</SelectItem>
                ...
                <div className="w-50">
                    <Select required value={filters.eventID} onValueChange={(value)=>{setFilters(prev => ({...prev, eventID: value}))}}>
      <SelectTrigger className="w-full">
        <SelectValue placeholder="Pick a Event" />
      </SelectTrigger>
      <SelectContent>
        <SelectGroup>
          ...
          {pagesData.map(item => (
            ...
            <SelectItem key={item.eventID} value={item.eventID}>{item.title}</SelectItem>
          ))}
        </SelectGroup>
      </SelectContent>
    </Select>
...
export default DashboardEventTrends