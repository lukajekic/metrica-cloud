...
const DashboardPageGeolocation = () => {
...
useEffect(()=>{
    ...
    const getStats = async()=>{
        try {
            ...
            console.log(popular)
            setMostPopular(popular.path)

            const monthstats = ProccessData(popular.path, response.data, firstdate )
            console.log('MESECNI:', monthstats)
setMonthStats({...monthstats, dateFancy: `${moment(firstdate).format('DD. MM. YYYY. HH:MM')} UTC`})
            const weekstats = ProccessData(popular.path, response.data, weekdayDeadline )
            console.log('NEDELJNI:', weekstats)
setWeekStats({...weekstats, dateFancy: `${moment(weekdayDeadline).format('DD. MM. YYYY. HH:MM')} UTC`})

            const dailystats = ProccessData(popular.path, response.data, lastdate )
...
let countrydominationlocal= {}
    ...
    const DetermineMostPopular = (data) =>{
        ...
        for (const item of data) {
            let totalViewsSum = Object.values(item.views)
            if (!pathsPopularity[item.path]) {
            pathsPopularity[item.path] = [];
        }
            pathsPopularity[item.path] = pathsPopularity[item.path].concat(totalViewsSum)
        }
        ...
        const pathrelated = data.filter(item => item.path === max)
    ...
    const ProccessData = (path, data, startdate)=> {
        
        if (!path || !data || !startdate) {
            return {
  total: 0,
  unique: 0
}
...
let filtered = data.filter((item)=>{
    ...
    return item.path === path && itemDate >= startdate
})
let TotalViewsSum = 0
let UniqueViewssum = 0
 ...
 if (StatsCountryFilter) {
    filtered.forEach(pageview => {
        TotalViewsSum += pageview.views?.[StatsCountryFilter] ?? 0
        UniqueViewssum += pageview.uniqueViews?.[StatsCountryFilter] ?? 0
    })

 } else {
    filtered.forEach(pageview => { 
        const internalTotalSum = Object.values(pageview.views).reduce((prev,current)=>{ return prev+current }) 
        TotalViewsSum += internalTotalSum 
        const internalUniqueSum = Object.values(pageview.uniqueViews).reduce((prev,current)=>{ return prev+current }) 
  ...
  return (
    <div id='dashboard-page-geolocation'>
        ...
        <MainContent enforceProtection>
            <DashboardPageTitle title='Page Views Geolocation' subtitle='Analytics showing the origin countries of all page visits for this project.'></DashboardPageTitle>
                    ...
                    <div className="col-span-4 row-span-2">
                        <DashboardBlock tailwindHeight='h-full' title={`Most Popular Today ${StatsCountryFilter ? (`(${StatsCountryFilter})`) : ('')}`}>
                            ...
                            <H1>{todayStats.unique}</H1>
                                ...
                                <P className='inline-flex items-center gap-2'>Calculated since {todayStats.dateFancy}<span><MetricaTooltip text='Based on your globally most popular page, filtered by your selected country.'></MetricaTooltip></span></P>
                            </div>
                        </DashboardBlock>
                    </div>
                    ...
                    <div className="col-span-2 row-span-2">
                        <DashboardBlock tailwindHeight='h-full' title={`Most Popular last 7 Days ${StatsCountryFilter ? (`(${StatsCountryFilter})`) : ('')}`}>
                            <div className="flex flex-col gap-2 h-full justify-evenly">
                                ...
                                <P className='inline-flex items-center gap-2'>Calculated since {weekStats.dateFancy}<span><MetricaTooltip text='Based on your globally most popular page, filtered by your selected country.'></MetricaTooltip></span></P>
                            </div>
                        </DashboardBlock>
                    </div>
                    ...
                    <div className="col-span-2 row-span-2">
                        <DashboardBlock tailwindHeight='h-full' title={`Most Popular last 30 Days ${StatsCountryFilter ? (`(${StatsCountryFilter})`) : ('')}`}>
                            <div className="flex flex-col gap-2 h-full justify-evenly">
                                ...
                                <P className='inline-flex items-center gap-2'>Calculated since {monthStats.dateFancy}<span><MetricaTooltip text='Based on your globally most popular page, filtered by your selected country.'></MetricaTooltip></span></P>
                            </div>
                        </DashboardBlock>
                    </div>
...
export default DashboardPageGeolocation